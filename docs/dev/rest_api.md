...
| Header 名称       | 必选性 | 核心作用                          | 通用值示例                                                                 | 备注                                  |
|--------------------|--------|-----------------------------------|----------------------------------------------------------------------------|---------------------------------------|
| `Authorization`    | 是     | 身份验证，确认请求者权限          | `Bearer b96810faac725563304dada8c323c4fa061863d4`                          | 值格式为 `Bearer + 空格 + 管理员Token`，所有 API 接口均需携带以验证权限  |
| `X-JMS-ORG`        | 否     | 指定操作所属组织（企业版功能）    | `00000000-0000-0000-0000-000000000002`                                      | 留空默认使用 `Default` 组织，用于多组织数据隔离  |
| `Content-Type`     | 是     | 指定请求体数据格式                | `application/json`                                                         | 所有 POST/PUT/PATCH 请求均需指定为 JSON 格式，确保服务器正确解析参数  |